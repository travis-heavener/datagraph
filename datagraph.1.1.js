...
function DataGraph(inputToDisplay, displayX=600, displayY=600, trendline=null, graphw=null, graphh=null) {
    ...
    this.draw = function() {
        ...
        this.gridIntervalX = Math.abs(maxX) > Math.abs(minX) ? Math.abs(maxX) * 2 : Math.abs(minX) * 2;
        this.gridIntervalY = Math.abs(maxY) > Math.abs(minY) ? Math.abs(maxY) * 2 : Math.abs(minY) * 2;
        
        this.gridIntervalX = this.screenWidth / this.gridIntervalX;
        this.gridIntervalY = this.screenHeight / this.gridIntervalY;
        
        let radius = 3;
        let unitX = this.gridIntervalX;
        let unitY = this.gridIntervalY;
        //--------plot points-----------
